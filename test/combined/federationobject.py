...
permissive_false_values = ['identifying']  #depends on AuthZ policy
...
class Obj():
    ...
    def test_create(self):
        ...
        self._check_true_and_false('create')
        ...
        if 'create' in self.methods:
            if 'lookup' in self.methods:
                ...
                if 'delete' in self.methods:
                    ...
                    self.logger.warn(
                        'Could not cleanup test; no way to delete test data!')
            else:
                self.logger.error(
                    'Could not test object; no way to lookup test data!')
                return False
            return True
        else:
            self.logger.error(
    ...
    def _check_true_and_false(self, method, urn=None):
        self._bundle_calls(method, self.permissive_true_values, None, 0,
                           urn=urn)
        self._bundle_calls(method, self.permissive_false_values, None, 3,
                           urn=urn)
